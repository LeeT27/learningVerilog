...
With interests in **embedded systems** and **digital design**, I wanted a head start with an **HDL** as the skill is fundamental for both ASIC design/verification.
...
## [Final Project: Single-Cycle CPU](https://github.com/LeeT27/learningVerilog/tree/main/Single-Cycle%20CPU)
...
This project took a **VERY LONG** time to debug and get each instruction action working. Here is a list of the biggest and most time-consuming problems that I encountered when building the CPU.
...
- I forgot to **match instance names** in the test bench so referring to RAM values like `cpu.f2.memory[0]` didn't correctly return
- Forgot to reset toggles `mem_read`, `mem_write`, `reg_write`, and `jump` at the beginning of each instruction, unintentionally leaving wrong toggles
...
- Conversion constants are in storage: in memory[92], memory[93], and memory[94]
  - memory[92]: 60
  - memory[93]: 262
  - memory[94]: 10
...
<img width="700" alt="image" src="https://github.com/user-attachments/assets/ee6f485e-3021-4e9e-a27a-6404a23f8c2e" />
